...
from libvirt_backup_system.preflight_estimate import (
    disk_virtual_size_bytes,
...
def test_disk_virtual_size_bytes_parses_qemu_img_json(monkeypatch) -> None:
    ...
    monkeypatch.setattr("libvirt_backup_system.preflight_estimate.run", fake_run)
    assert disk_virtual_size_bytes("/x.qcow2") == 4294967296


def test_vm_estimated_bytes_falls_back_for_remote_uri_without_local_probe(monkeypatch, capsys) -> None:
    monkeypatch.setattr(
        "libvirt_backup_system.preflight_estimate.vm_disk_paths",
        lambda uri, name: pytest.fail("remote disk list must not be queried locally"),
    )
    monkeypatch.setattr(
        "libvirt_backup_system.preflight_estimate.disk_virtual_size_bytes",
        lambda path: pytest.fail("qemu-img must not run on remote disk paths"),
    )
    assert vm_estimated_bytes("qemu+ssh://host/system", VM("alpha", "running", ALPHA_UUID), 11) == 11
...
def test_vm_estimated_bytes_uses_fallback_when_virsh_fails(monkeypatch, capsys) -> None:
    ...
    assert vm_estimated_bytes("qemu:///system", VM("alpha", "running", ALPHA_UUID), 5) == 5
...
def test_vm_estimated_bytes_uses_fallback_when_qemu_img_fails(monkeypatch, capsys) -> None:
    ...
    assert vm_estimated_bytes("qemu:///system", VM("alpha", "running", ALPHA_UUID), 7) == 7
...
def test_vm_estimated_bytes_adds_fallback_per_failed_disk(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr("libvirt_backup_system.preflight_estimate.disk_virtual_size_bytes", fake_disk_size)
    assert vm_estimated_bytes("qemu:///system", VM("alpha", "running", ALPHA_UUID), 7) == 107
    assert "qemu-img info failed for disk" in capsys.readouterr().err
...
def test_vm_estimated_bytes_adds_fallback_for_each_failed_disk(monkeypatch) -> None:
    ...
    assert vm_estimated_bytes("qemu:///system", VM("alpha", "running", ALPHA_UUID), 11) == 33
...
def test_vm_estimated_bytes_sums_disks(monkeypatch) -> None:
    ...
    assert vm_estimated_bytes("qemu:///system", VM("alpha", "running", ALPHA_UUID), 99) == 3500
...
def test_vm_estimated_bytes_empty_disk_list_falls_back(monkeypatch) -> None:
    ...
    assert vm_estimated_bytes("qemu:///system", VM("alpha", "running", ALPHA_UUID), 42) == 42


def test_estimate_required_kb_handles_bad_floats(backup_config) -> None:
    cfg = backup_config
    cfg.values["BACKUP_ESTIMATE_GB_PER_VM"] = "bad"
    assert estimate_required_kb(cfg, [VM("alpha", "running", ALPHA_UUID)]) == 0
