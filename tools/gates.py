...
IGNORED_DIRS = {
    ...
    "build",
    "dist",
    # Design docs are intentionally long-form prose and exempt from the LOC
    # gate — the gate exists to bound the cost of *reading code*, not to
    # constrain how much context an architecture plan needs.
    "plans",
}
# Suffixes for tree directories that are not source code and should be skipped
# from the LOC gate. ``.egg-info`` is generated by ``pip install -e .`` and only
# appears on developer hosts; without this skip a local install trips LOC
# failures that CI (running on a fresh clone) never sees.
...
def text_files() -> list[Path]:
    ...
    for path in ROOT.rglob("*"):
        ...
        if any(part in IGNORED_DIRS for part in parts):
            continue
        if any(part.endswith(IGNORED_DIR_SUFFIXES) for part in parts):
            continue
